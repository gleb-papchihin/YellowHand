...
def fit_bisenet(batches, preprocessor, model, optimizer, loss_fn, loss_aux, epochs,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i in range(start_batch, len(batches)):
            ...
            # Step
            optimizer.zero_grad()
            pred, aux16, aux32 = model(x)
            loss_p = loss_fn(pred, y)
            loss_aux_0 = loss_aux[0](aux16, y)
            loss_aux_1 = loss_aux[1](aux32, y)
            loss = loss_p + loss_aux_0 + loss_aux_1
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    resize = (480, 640)
    model = BiSeNet(n_classes=1)
    loss_fn = nn.BCEWithLogitsLoss()
    loss_aux_1 = nn.BCEWithLogitsLoss()
    loss_aux_2 = nn.BCEWithLogitsLoss()
    loss_aux = [loss_aux_1, loss_aux_2]
    ...
    for i in range(meta["epoch"], 5):
        ...
        fit_bisenet(batches, preprocessor, model, optimizer, loss_fn, loss_aux, (i+1), 
